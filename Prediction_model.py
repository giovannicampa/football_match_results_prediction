...
neural_network = MLPRegressor(activation = 'logistic')

param_grid_rf = {'max_depth':list(range(2,10)), 'max_features': list(range(5,X.shape[1])), 'n_estimators': list(range(70,200,10))}
param_grid_mlp = {'hidden_layer_sizes': list(range(20,100,7))}
...
feature_relevance = (regressor_rf.feature_importances_>0.01)
...
print('Prediction accuracy with most relevant features: ',sum(predicted_winners.reshape(1,-1)[0] == real_winners)/nr_games*100)